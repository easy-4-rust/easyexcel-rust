...
use crate::writer::builder::excel_writer_table_builder::merge_table_options;
use crate::writer::executor::excel_write_fill_executor::ExcelWriteFillExecutor;
use crate::writer::metadata::WriteTable;
...
impl ExcelBuilderImpl {
        ...
        T: ExcelRow,
    {
        ...
        self.context.set_current_holder_state(
            crate::writer::resolved_write_context_holder_state::<T>(options, table_no)?,
        );
        Ok(())
    }
...
mod tests {
    ...
    fn fill_uses_explicit_excel_type_instead_of_path_extension() {
        let mut builder = ExcelBuilderImpl::from_options(
            "logical.xlsx",
            WriteOptions {
                excel_type: Some(crate::core::support::ExcelTypeEnum::Csv),
    ...
    fn write_context_exposes_sheet_and_table_after_add_content() -> Result<()> {
        ...
        let sheet = WriteSheet::<DynamicRow>::new(" Sheet1 ");
        let table = crate::writer::ExcelWriterTableBuilder::new()
            .table_no(1)
            .need_head(false)
            .build();
        let mut builder = ExcelBuilderImpl::from_options(&path, WriteOptions::default());
    ...
    fn live_current_write_holder_tracks_resolved_sheet_and_table_state() -> Result<()> {
        ...
        assert_eq!(
            sheet_holder
                ...
                .map(crate::core::metadata::RowHeightProperty::height),
            Some(28)
        );
        assert_eq!(
            sheet_holder
    ...
    fn template_fill_updates_the_same_live_current_holder() -> Result<()> {
        ...
        assert_eq!(
            holder
                ...
                .head_map()
                .values()
                .flat_map(crate::core::metadata::Head::head_name_list)
                .map(String::as_str)
                .collect::<Vec<_>>(),
            vec!["填充列"]
        );
    ...
    fn fill_legacy_xls_is_rejected() {
        let mut builder = ExcelBuilderImpl::from_options(
            "legacy.xls",
            WriteOptions {
                excel_type: Some(crate::core::support::ExcelTypeEnum::Xls),
                template_bytes: Some(vec![1]),
                ..WriteOptions::default()
            },
        );
