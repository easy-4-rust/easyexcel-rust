...
fn writer_global_flags_default() {
    ...
    let flags = crate::writer::writer_helpers::WriteGlobalFlags::from(&options);
...
fn effective_sheet_name_no_trim() {
    ...
    let name = crate::writer::writer_helpers::effective_sheet_name(&options);
...
fn maybe_trim_cell_string_no_trim() {
    let result = crate::writer::writer_helpers::maybe_trim_cell_string("  hello  ", false);
    assert_eq!(result, "  hello  ");
}

#[test]
fn maybe_trim_cell_string_with_trim() {
    let result = crate::writer::writer_helpers::maybe_trim_cell_string("  hello  ", true);
    assert_eq!(result, "hello");
}

#[test]
fn is_scientific_magnitude_large() {
    assert!(crate::writer::writer_helpers::is_scientific_magnitude(
        1.5e15
    ));
    assert!(crate::writer::writer_helpers::is_scientific_magnitude(
        -1.5e15
...
fn is_scientific_magnitude_normal() {
    ...
    assert!(!crate::writer::writer_helpers::is_scientific_magnitude(0.0));
    assert!(!crate::writer::writer_helpers::is_scientific_magnitude(1.0));
}

#[test]
fn share_handlers_empty() {
    let handlers: Vec<Box<dyn WriteHandler>> = vec![];
    let shared = crate::writer::writer_helpers::share_handlers(handlers);
...
fn boxed_handlers_roundtrip() {
    ...
    let shared = crate::writer::writer_helpers::share_handlers(handlers);
    let boxed = crate::writer::writer_helpers::boxed_handlers(&shared);
...
fn handler_execution_scope_root() {
    let handlers = DefaultWriteHandlerLoader::load_default_handler();
    let shared = crate::writer::writer_helpers::share_handlers(handlers);
    let scope = crate::writer::writer_helpers::HandlerExecutionScope::root(&shared);
...
fn handler_execution_scope_default() {
    let scope = crate::writer::writer_helpers::HandlerExecutionScope::default();
    assert!(scope.own.is_empty());
    assert!(scope.effective.is_empty());
}
...
fn captured_output_default() {
    let output = crate::writer::writer_helpers::CapturedOutput::default();
    let bytes = crate::writer::writer_helpers::take_captured_output_helper(&output);
...
fn normalized_shared_handlers_sorts_by_order_and_dedups() {
    ...
    let shared = crate::writer::writer_helpers::share_handlers(vec![
        Box::new(HandlerA),
        Box::new(HandlerB),
        Box::new(HandlerC),
    ]);
    let normalized = crate::writer::writer_helpers::normalized_shared_handlers(shared);
...
fn handler_execution_scope_child() {
    let handlers = DefaultWriteHandlerLoader::load_default_handler();
    let shared = crate::writer::writer_helpers::share_handlers(handlers);
    let root = crate::writer::writer_helpers::HandlerExecutionScope::root(&shared);
    let child = crate::writer::writer_helpers::HandlerExecutionScope::child(&shared, &root);
...
fn handler_execution_scope_child_inherits_parent() {
    ...
    let shared = crate::writer::writer_helpers::share_handlers(handlers);
    let root = crate::writer::writer_helpers::HandlerExecutionScope::root(&shared);
    let child = crate::writer::writer_helpers::HandlerExecutionScope::child(&[], &root);
...
fn stateful_sheet_state_construction() {
    ...
    use crate::writer::metadata::write_sheet::WriteSheet as MirroredWriteSheet;
    let _sheet = MirroredWriteSheet::new();
    let options = WriteOptions::default();
    let metadata = ExcelWriteMetadata::default();
    let _state = crate::writer::writer_helpers::StatefulSheetState {
        schema: &[],
        metadata,
        options,
        next_row: 0,
...
fn shared_write_handler_clone() {
    ...
    let shared = crate::writer::writer_helpers::share_handlers(vec![handler]);
