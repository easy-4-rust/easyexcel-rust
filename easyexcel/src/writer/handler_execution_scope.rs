...
use crate::writer::excel_writer_core::{selected_columns, to_column};
use crate::writer::merge::loop_merge_strategy::LoopMergeStrategy as MirroredLoopMergeStrategy;
use crate::writer::merge::once_absolute_merge_strategy::OnceAbsoluteMergeStrategy;
...
use crate::writer::style::column::simple_column_width_style_strategy::SimpleColumnWidthStyleStrategy;
use crate::writer::style::row::simple_row_height_style_strategy::SimpleRowHeightStyleStrategy;
...
impl WriteHandler for AnnotationCellStyleHandler {
    fn order(&self) -> i32 {
        crate::core::constant::order_constant::ANNOTATION_DEFINE_STYLE
    }
...
pub(crate) fn ensure_gzip_spill<'a>(
    spills: &'a mut HashMap<String, crate::writer::gzip_spill::GzipSheetDataWriter>,
    sheet_name: &str,
    compress: bool,
) -> Result<Option<&'a mut crate::writer::gzip_spill::GzipSheetDataWriter>> {
    if !compress {
        return Ok(None);
    }
    if !spills.contains_key(sheet_name) {
        spills.insert(
            sheet_name.to_owned(),
            crate::writer::gzip_spill::GzipSheetDataWriter::create_owned(sheet_name)?,
        );
    }
