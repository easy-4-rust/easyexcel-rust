...
///   [`crate::writer::biff8::Biff8TemplatePackage`] instead (see writer `start` / `write_xls`).
...
fn render_cell_xml(reference: &str, value: &CellValue, style: Option<u32>) -> String {
    ...
    match value {
        ...
        CellValue::Decimal(number) => {
            if crate::writer::decimal_integer_requires_text(number).unwrap_or(false) {
                format!(
                    "<c r=\"{reference}\"{style_attribute} t=\"inlineStr\"><is><t>{}</t></is></c>",
