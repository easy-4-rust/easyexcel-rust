...
use crate::core::util::work_book_util::create_sheet;
use crate::core::{
...
use crate::writer::biff8::Biff8Book;
use crate::writer::csv_encoding_writer::CsvEncodingWriter;
use crate::writer::excel_output_stream::ExcelOutputStream;
use crate::writer::excel_writer_core::{
...
use crate::writer::handler::default_write_handler_loader::DefaultWriteHandlerLoader;
use crate::writer::handler_execution_scope::{
    HandlerExecutionScope, ensure_gzip_spill, load_annotation_handlers,
};
use crate::writer::metadata::write_table::WriteTable as MirroredWriteTable;
use crate::writer::shared_write_handler::{
    SharedWriteHandler, StatefulSheetState, boxed_handlers, share_handlers,
};
use crate::writer::write_options::WriteOptions;
use crate::writer::write_progress::WriteProgress;
use crate::writer::write_sheet::WriteSheet;

/// Stateful XLSX or single-sheet CSV writer matching Java `ExcelWriter`'s lifecycle.
#[allow(clippy::struct_excessive_bools)]
pub struct ExcelWriter {
    path: PathBuf,
    excel_type: Option<crate::core::support::ExcelTypeEnum>,
    ...
    template_package: Option<crate::writer::template_write::TemplatePackage>,
    ...
    /// records are copied verbatim ([`crate::writer::biff8::Biff8TemplatePackage`]).
    xls_template: Option<crate::writer::biff8::Biff8TemplatePackage>,
    ...
    pub(crate) gzip_spills: HashMap<String, crate::writer::gzip_spill::GzipSheetDataWriter>,
    /// Last finished gzip spill snapshot (for tests / observability).
    last_gzip_spill: Option<crate::writer::gzip_spill::GzipSpillSnapshot>,
}
...
impl ExcelWriter {
    ...
    /// sheet's options via [`crate::writer::builder::excel_writer_table_builder::merge_table_options`].
    ///
        ...
        I: IntoIterator<Item = T>,
    {
        ...
        validate_excel_row_schema::<T>()?;
        self.start()?;
        let merged = crate::writer::builder::excel_writer_table_builder::merge_table_options(
            sheet.options(),
            table,
        );
    ...
    pub fn has_template_configured(&self) -> bool {
        crate::writer::template_write::has_template(
    ...
    ///
    /// Populated when [`Self::finish`] closes active [`crate::writer::gzip_spill::GzipSheetDataWriter`]s.
    #[must_use]
    pub const fn last_gzip_spill_snapshot(
        &self,
    ) -> Option<&crate::writer::gzip_spill::GzipSpillSnapshot> {
    ...
    pub(crate) fn start(&mut self) -> Result<()> {
        ...
        if crate::writer::template_write::has_template(
            self.template_file.as_deref(),
            self.template_bytes.as_deref(),
        ) {
            if self.is_csv() {
            ...
            if self.is_xls() {
                ...
                let package = crate::writer::biff8::Biff8TemplatePackage::from_bytes(&bytes)?;
                ...
                if self.use_legacy_template_seed {
                    ...
                    crate::writer::template_write::seed_workbook_from_template(
                        &mut self.workbook,
                        &sheets,
                    )?;
                    ...
                    let package =
                        crate::writer::template_write::TemplatePackage::from_bytes(&bytes)?;
                    for (index, name) in package.sheet_names()?.into_iter().enumerate() {
                        let next_row = package.next_row_for_sheet(&name)?;
                        self.sheet_indexes.insert(index, name.clone());
                        self.template_pending_rows.insert(name, next_row);
