...
fn cell_value_to_template_cell(value: &CellValue) -> Result<Biff8Cell> {
    let mapped = match value {
        ...
        CellValue::Decimal(number) => {
            let numeric = crate::writer::finite_decimal_f64(number, "BIFF8")?;
            if crate::writer::decimal_integer_requires_text(number)? {
...
mod tests {
    use super::*;
    use crate::writer::biff8::{Biff8Book, Biff8Cell, Biff8Value};
    ...
    fn cell_value_to_template_cell_maps_all_variants() {
        ...
        let rich = cell_value_to_template_cell(&CellValue::RichText(
            crate::core::RichTextStringData::new("rich"),
        ))
        .unwrap();
