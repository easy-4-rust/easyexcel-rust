...
<Relationship Id="comments" Type="urn:relationships/comments" Target="../comments1.xml"/>
    ...
    assert_eq!(extras[0].extra_type(), crate::core::CellExtraType::Merge);
    ...
    assert_eq!(extras[3].extra_type(), crate::core::CellExtraType::Comment);
...
fn extra_parsers_report_missing_relationships_attributes_parts_and_xml() {
    ...
    assert!(
        metadata
            .extras("A&B", &HashSet::from([crate::core::CellExtraType::Comment]))
            .is_err()
    );
    ...
    assert!(
        metadata
            .extras(
                "A&B",
                &HashSet::from([crate::core::CellExtraType::Hyperlink])
            )
            .is_err()
    );
    let mut metadata =
