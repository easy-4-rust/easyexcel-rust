...
use crate::reader::analysis::excel_read_executor::ExcelReadExecutor;
use crate::reader::analysis::v03::biff_record_stream::{read_workbook_stream, walk_biff_records};
use crate::reader::analysis::v03::xls_list_sheet_listener::XlsListSheetListener;
use crate::reader::analysis::v03::xls_record_dispatcher::{
    XlsRecordDispatchState, XlsRecordDispatcher,
};
use crate::reader::context::{DefaultXlsReadContext, ReadSheet, XlsReadContext};
...
mod tests {
    ...
    fn list_xls_sheets_matches_analyser_sheet_list() -> Result<()> {
        let file = write_java_multisheet_xls();
        let options = ReadOptions::default();
        let discovered = crate::reader::list_xls_sheets(file.path(), &options)?;
        let analyser = XlsSaxAnalyser::from_path(file.path(), options)?;
    ...
    fn empty_workbook_path_fails_sheet_discovery() {
        ...
        assert!(crate::reader::list_xls_sheets(&path, &ReadOptions::default()).is_err());
    }
}
