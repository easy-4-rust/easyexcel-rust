//! 公式缓存值求值桥接。
//!
//! 借用本地 fork 的 `xls` crate 公式求值引擎（MIT OR Apache-2.0，
//! `xls::core::formula::Engine`）在写入时当场计算 FORMULA 记录缓存值，
...
use xls::core::{Cell, CellError, CellValue, Workbook};
...
mod tests {
    use super::*;
    use crate::write::biff8::workbook::{Biff8Cell, Biff8Sheet};
