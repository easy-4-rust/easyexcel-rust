...
| `readCache` / built-in `ReadCacheSelector` | `read_cache(ReadCacheMode::{Auto, Memory, Moka, File})` | implemented: `Auto` matches the 5,000,000-byte `sharedStrings.xml` boundary; large SAX reads use the UTF-8 temporary-file backend, while explicit Moka mode retains objects without entry eviction |
...
stores indexed UTF-8 strings in a vector; `File` stores bytes in a temporary
file and retains only offsets and lengths in memory; `Auto` chooses between
them. `Moka` is an explicit lifecycle object cache with no capacity or time
eviction. The backends are exercised against the same shared-string, formula,
number, and date fixture. Java's `XlsxSaxAnalyser` does not map drawing
