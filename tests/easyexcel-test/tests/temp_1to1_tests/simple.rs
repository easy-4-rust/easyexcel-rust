...
use easyexcel::{
    CellValue, EasyExcel, ExcelCellStyle, ExcelColor, ExcelDataFormat, ExcelRow, WriteHandler,
...
/// Java `com.alibaba.easyexcel.test.temp.simple.Write#simpleWrite1` —
/// `BeanMapUtils.create(LargeData)` key probes.
///
#[test]
...
/// Java `com.alibaba.easyexcel.test.temp.simple.Write#simpleWrite` —
/// `relativeHeadRowIndex(10)` + `DemoData` write.
#[test]
...
/// Java `com.alibaba.easyexcel.test.temp.simple.Write#simpleWrite2` —
/// `WriteData` + `SheetWriteHandler` `protectSheet`.
///
/// Rust 通过 handler 的后端中立修改计划在保存前应用工作表保护。
#[test]
fn simple_write_simple_write_2() {
    ...
    impl WriteHandler for ProtectProbeHandler {
        fn after_sheet(&mut self, ctx: &WriteSheetContext) -> easyexcel::Result<()> {
            // Java: writeSheetHolder.getSheet().protectSheet("edit")
            self.hits.fetch_add(1, Ordering::Relaxed);
            ctx.protect_sheet("edit")
        }
    }
    #[derive(Debug, Clone, ExcelRow)]
    struct WriteData {
    ...
    let worksheet = zip_entry_text(&path, "xl/worksheets/sheet1.xml");
    assert!(worksheet.contains("<sheetProtection"));
    assert!(worksheet.contains("password=\""));
}

/// Java `com.alibaba.easyexcel.test.temp.simple.Write#simpleWrite3` —
/// dynamic head + `inMemory(true)` + `CellWriteHandler`.
///
/// Rust default XLSX write is already in-memory (non-`constant_memory`).
/// POI `Cell.setCellStyle` / `IndexedColors` 由 `style_cell_style` 表达，并读取
/// 生成的 OOXML 校验格式确实落盘。
#[test]
fn simple_write_simple_write_3() {
    ...
    let styles = zip_entry_text(&path, "xl/styles.xml");
    assert!(styles.contains("yyyy-MM-dd"));
    assert!(styles.contains("wrapText=\"1\""));
    let worksheet = zip_entry_text(&path, "xl/worksheets/sheet1.xml");
    assert!(
        worksheet.contains("r=\"A2\" s=\"") && worksheet.contains("r=\"B2\" s=\""),
        "data cells must reference a generated style: {worksheet}"
    );
}

fn zip_entry_text(path: &std::path::Path, name: &str) -> String {
    let file = File::open(path).expect("open generated xlsx");
    let mut archive = zip::ZipArchive::new(file).expect("open xlsx zip");
...
/// Java `com.alibaba.easyexcel.test.temp.simple.Write#json` — Fastjson2 serialize
/// `JsonData` (SS1/sS2/ss3).
#[test]
...
/// Java `com.alibaba.easyexcel.test.temp.simple.Write#json3` — Fastjson2 parse
/// then re-serialize `JsonData`.
#[test]
...
/// Java `com.alibaba.easyexcel.test.temp.simple.Write#tableWrite` —
/// `writerTable(0).head(DemoData1)` single table write.
///
/// Rust exposes the Java three-argument overload as `write_with_table`.
#[test]
fn simple_write_table_write() {
    ...
    let mut table = easyexcel::write::metadata::WriteTable::with_table_no(0);
    table.parameter.need_head = Some(true);
    writer.write_with_table(data, &sheet, &table).unwrap();
    writer.finish().unwrap();
