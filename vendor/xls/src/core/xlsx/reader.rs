...
use crate::core::error::{CellError, Error, Result};
...
pub fn read_with_password<R: Read + Seek>(
    ...
    if crate::core::xls::looks_like_cfb(&magic[..n]) {
    ...
    loop {
        match reader.read_event_into(&mut buf)? {
            ...
            Event::Text(t) if in_defined_name => {
                cur_name_text.push_str(&t.unescape().unwrap_or_default());
            }
...
mod tests {
    use super::*;
    use crate::core::styles::{CellStyle, HAlign};
    use crate::core::xlsx::write;
