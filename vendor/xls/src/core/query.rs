...
use crate::core::addr::col_letters_to_index;
use crate::core::formula::coerce::parse_number_text;
use crate::core::value::{CellValue, format_number_general};
...
impl Parser {
    ...
    fn parse_select(&mut self) -> Result<SelectStmt> {
            ...
            loop {
                let by = match self.peek() {
                    Some(Tok::Num(n)) => {
                        let n = *n;
                        self.next();
                        OrderBy::Ordinal(n as usize)
                    }
                    _ => {
                        let cr = self.parse_colref()?;
                        OrderBy::Name(cr.name)
                    }
                };
        ...
        let limit = if self.eat_kw("LIMIT") {
            match self.next() {
                Some(Tok::Num(n)) => Some(n as usize),
...
impl Table {
    fn from_sheet(wb: &Workbook, idx: usize, alias: String) -> Table {
    ...
    fn col_index(&self, name: &str) -> Option<usize> {
        ...
        if !name.is_empty()
            ...
            && let Some(ci) = col_letters_to_index(&name.to_ascii_uppercase())
            && (ci as usize) < self.ncols
        {
    ...
    fn header_name(&self, ci: usize) -> String {
        ...
        if h.is_empty() {
            crate::core::addr::col_index_to_letters(ci as u32)
        } else {
            h
        }
    }
...
fn aggregate(
    ...
    match f {
        AggFn::Count => match arg {
            None => CellValue::Number(members.len() as f64),
            Some((ti, ci)) => {
                let n = members
                    .iter()
        ...
        AggFn::Sum | AggFn::Avg => {
            let Some((ti, ci)) = arg else {
                return CellValue::Error(crate::core::error::CellError::Value);
            };
            ...
            match f {
                ...
                _ => CellValue::Number(sum / nums.len() as f64),
            }
        }
        AggFn::Min | AggFn::Max => {
            let Some((ti, ci)) = arg else {
