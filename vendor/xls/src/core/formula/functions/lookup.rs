...
use super::Registry;
use crate::core::addr::{CellAddress, col_index_to_letters};
use crate::core::error::CellError;
use crate::core::formula::coerce::{compare, to_number, to_text};
use crate::core::formula::context::Context;
use crate::core::formula::value::{Array, RefRange, Value};
...
fn vlookup(ctx: &mut dyn Context, args: &[Value]) -> Value {
    ...
    let approx = if args.len() >= 4 {
        match crate::core::formula::coerce::to_bool(&scalar(ctx, &args[3])) {
            Ok(b) => b,
            Err(e) => return Value::Error(e),
        }
    } else {
...
fn indirect(ctx: &mut dyn Context, args: &[Value]) -> Value {
    ...
    let a1_style = if args.len() >= 2 {
        match crate::core::formula::coerce::to_bool(&scalar(ctx, &args[1])) {
            Ok(b) => b,
            Err(e) => return Value::Error(e),
        }
    } else {
    ...
    if a1_style {
        if let Some(cr) = crate::core::addr::CellRange::parse_a1(&cell_part) {
...
fn address_fn(_ctx: &mut dyn Context, args: &[Value]) -> Value {
    ...
    let _a1_style = if args.len() >= 4 {
        match crate::core::formula::coerce::to_bool(&args[3]) {
            Ok(b) => b,
            Err(e) => return Value::Error(e),
        }
    } else {
