...
use crate::core::model::{Cell, Spill, Workbook};
    ...
    pub fn recalc(&mut self, wb: &mut Workbook) -> RecalcReport {
        ...
        for node in in_cycle {
            ...
            write_cached(
                ...
                row,
                col,
                crate::core::value::CellValue::Error(CellError::Ref),
            );
        }
...
mod tests {
    ...
    fn set_formula(wb: &mut Workbook, a1: &str, f: &str) {
        wb.sheet_mut(0).unwrap().set_a1(
            ...
            Cell::Formula {
                expr: f.to_string(),
                cached: crate::core::value::CellValue::Empty,
            },
        );
    }
    ...
    fn chained_recalc() {
        ...
        assert_eq!(
            ...
            crate::core::value::CellValue::Number(20.0)
        );
        assert_eq!(
            wb.sheet_mut(0).unwrap().value(2, 0),
            crate::core::value::CellValue::Number(25.0)
        );
    }
