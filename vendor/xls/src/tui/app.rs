...
use crate::core::addr::{CellAddress, CellRange, col_index_to_letters};
use crate::core::formula::Engine;
use crate::core::{Cell, Workbook};
...
impl App {
    ...
    pub fn sheet(&self) -> &crate::core::Sheet {
    ...
    pub fn coerce_selection_to_numbers(&mut self) -> usize {
        ...
        for (r, c) in sel.iter_cells() {
            if let Some(Cell::Text(s)) = self.wb.sheets[si].get(r, c)
                && let Some(n) = crate::core::formula::coerce::parse_number_text(s)
            {
    ...
    pub fn save_to(&mut self, path: &std::path::Path) -> anyhow::Result<()> {
        crate::core::save_path(&self.wb, path)?;
