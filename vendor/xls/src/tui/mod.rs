...
pub fn run_path(path: PathBuf) -> anyhow::Result<()> {
    let wb = crate::core::open_path(&path)?;
...
fn sort_selection(app: &mut App, rest: &str) {
    ...
    let mut rows: Vec<Vec<crate::core::Cell>> = Vec::new();
        ...
        for c in sel.start.col..=sel.end.col {
            row.push(
                app.wb.sheets[si]
                    .get(r, c)
                    .cloned()
                    .unwrap_or(crate::core::Cell::Empty),
            );
        }
        rows.push(row);
    }
...
fn sort_key(v: &crate::core::CellValue) -> f64 {
    match v {
        crate::core::CellValue::Number(n) => *n,
        crate::core::CellValue::Bool(b) => {
            if *b {
                1.0
            } else {
                0.0
        ...
        crate::core::CellValue::Empty => f64::INFINITY,
        // Text/Error: sort lexically-ish by first char to keep it stable.
        crate::core::CellValue::Text(s) => 1e18 + s.bytes().next().map(|b| b as f64).unwrap_or(0.0),
        crate::core::CellValue::Error(_) => 2e18,
    }
}
