...
use crate::core::stream::{RowSink, StreamCell, StreamInfo};
    ...
    match &c.value {
        CellValue::Number(n) => {
            let is_date = !c.number_format.is_empty()
                && crate::core::numfmt::is_date_format(&c.number_format);
            if is_date {
                match opts.dates {
...
impl<W: Write> RowSink for CsvSink<W> {
    fn begin(&mut self, info: &StreamInfo) -> crate::core::error::Result<()> {
        self.date_system = info.date_system;
        Ok(())
    }

    fn row(&mut self, _row: u32, cells: &[StreamCell]) -> crate::core::error::Result<()> {
    ...
    fn end(&mut self) -> crate::core::error::Result<()> {
        self.wtr.flush().map_err(crate::core::error::Error::from)?;
        Ok(())
    }
}
...
impl<W: Write> JsonlSink<W> {
    ...
    fn capture_header(&self, cells: &[StreamCell]) -> Vec<String> {
        ...
        let mut names: Vec<String> = (0..n as u32)
            .map(crate::core::addr::col_index_to_letters)
            .collect();
        for c in cells {
