...
use crate::core;
use crate::core::formula::{CellRef, Engine};
...
fn run(cli: Cli) -> anyhow::Result<ExitCode> {
    ...
    match command {
        ...
        None => {
            ...
            {
                if let Some(file) = &top_file {
                    let wb = open_file_or_stdin(file)?;
                    cmd_info(&wb, file);
                    return Ok(ExitCode::SUCCESS);
                }
            }
        ...
        Value::Ref(r) => {
            ...
            for (rr, cc) in r.iter() {
                let cv = wb
                    ...
                    .get(r.sheet)
                    .map(|s| s.value(rr, cc))
                    .unwrap_or(crate::core::value::CellValue::Empty);
...
pub fn cmd_to_date(
    ...
    for (r, c) in rng.iter_cells() {
        let serial = match wb.sheets[idx].get(r, c) {
            Some(Cell::Text(s)) => crate::core::dates::parse_text_date(s, fmt, system),
            _ => None,
        };
...
pub fn cmd_pivot(
    ...
    for (k, (count, sum, min, max)) in &groups {
        ...
        out.push_str(&format!(
            "{k}\t{}\n",
            crate::core::value::format_number_general(v)
        ));
    }
...
pub fn cmd_profile(wb: &Workbook, column: &str, sheet: Option<&str>) -> anyhow::Result<String> {
    ...
    for r in 1..rows {
        ...
        match v {
            crate::core::value::CellValue::Empty => {
                nulls += 1;
                continue;
            }
            crate::core::value::CellValue::Number(n) => {
                numeric += 1;
                sum += n;
                min = min.min(n);
                max = max.max(n);
            }
            crate::core::value::CellValue::Text(ref t) => {
                text += 1;
                if crate::core::formula::coerce::parse_number_text(t).is_some() {
                    text_numbers += 1;
                } else if crate::core::dates::looks_like_date(t) {
                    text_dates += 1;
                }
            }
            _ => {}
    ...
    if numeric > 0 {
        let g = crate::core::value::format_number_general;
...
pub fn cmd_style(
    ...
    use crate::core::styles::{Color, FillPattern};
    let idx = resolve_sheet_idx(wb, sheet)?;
    let rng = parse_range(wb, range)?;
...
mod tests {
    use super::*;
    use crate::core::csv::{CsvWriteOptions, write_csv};
    use crate::core::model::{Cell, Workbook};
    use std::io::Write;
    ...
    fn to_date_converts_and_formats() {
        ...
        let serial = crate::core::dates::ymd_to_serial(wb.date_system, 2025, 4, 4).unwrap();
        ...
        assert_eq!(
            ...
            crate::core::value::format_number_general(serial)
        );
    }
    ...
    fn style_applies_bold_and_color() {
        ...
        assert!(st.font.bold);
        assert_eq!(st.font.color, crate::core::styles::Color::rgb(0xFFFF_0000));
    }
