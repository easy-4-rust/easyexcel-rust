//! 启动 `easyexcel-demo-actix` 服务进程，用原生 HTTP 请求验证
//! 下载 / 失败降级 / 上传三个端点（对应 Java `WebTest` 的三个接口）。
//!
//! 端口经 PORT 环境变量配置（对应 Java 可配置 quarkus.http.port），测试使用随机空闲端口。
...
fn demo_actix_serves_download_upload_and_error_endpoints() {
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_easyexcel-demo-actix"))
        .env("PORT", port)
        .spawn()
        .expect("spawn");
    let mut guard = ChildGuard(child);
    wait_ready(&mut guard.0, &address);
    // 服务刚就绪时 actix 的 accept 可能仍在预热；等待一个固定窗口
    ...
    // POST /upload → 垃圾字节：读取解析失败 → 500（对应 Java 上传解析异常）
    let garbage = format!(
    ...
    // POST /upload → 无边界前缀的畸形流：multipart 流项解析错误 → 400
    let (status, _, _) = http_request(
        &address,
        "POST",
        "/upload",
        &format!("Content-Type: multipart/form-data; boundary={BOUNDARY}\r\n"),
        b"not-a-boundary-prefix".as_slice(),
    );
    assert!(status.starts_with("HTTP/1.1 400"), "{status}");
