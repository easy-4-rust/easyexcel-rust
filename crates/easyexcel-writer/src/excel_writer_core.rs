...
pub use easyexcel_core::event::NotRepeatExecutor;
pub use easyexcel_core::metadata::csv::{CsvSheet, CsvWorkbook};
pub use easyexcel_core::util::work_book_util::{
...
pub use crate::builder::abstract_excel_writer_parameter_builder::AbstractExcelWriterParameterBuilder;
pub use crate::builder::excel_writer_table_builder::ExcelWriterTableBuilder;
pub use crate::cell_style::CellStyle;
...
pub use crate::executor::excel_write_fill_executor::ExcelWriteFillExecutor;
pub use crate::global_configuration::{
...
pub use crate::handler::r#impl::impl_default_row_write_handler::{
    DefaultRowWriteHandler, new_default_row_write_handler,
};
pub use crate::handler::r#impl::impl_dimension_workbook_write_handler::DimensionWorkbookWriteHandler;
pub use crate::handler::r#impl::impl_fill_style_cell_write_handler::FillStyleCellWriteHandler;
pub use crate::handler::row_write_handler::RowWriteHandler;
pub use crate::handler::sheet_write_handler::SheetWriteHandler;
pub use crate::handler::workbook_write_handler::WorkbookWriteHandler;
pub use crate::holder::abstract_write_holder::AbstractWriteHolder;
pub use crate::holder::write_holder::WriteHolder;
...
use crate::metadata::style::write_cell_style::merge_write_cell_style;
use crate::metadata::style::write_font::merge_excel_font_style as merge_handler_font_style;
pub use crate::metadata::style::write_font::{
    excel_font_style_from_write_font, merge_excel_font_style, merge_write_font,
};
pub use crate::metadata::write_basic_parameter::WriteBasicParameter as MirroredWriteBasicParameter;
pub use crate::metadata::write_sheet::WriteSheet as MirroredWriteSheet;
pub use crate::metadata::write_table::WriteTable as MirroredWriteTable;
pub use crate::metadata::write_workbook::WriteWorkbook as MirroredWriteWorkbook;
pub use crate::property::excel_write_head_property::ExcelWriteHeadProperty;
pub use crate::style::abstract_cell_style_strategy::AbstractCellStyleStrategy;
pub use crate::style::abstract_vertical_cell_style_strategy::AbstractVerticalCellStyleStrategy;
pub use crate::style::column::longest_match_column_width_style_strategy::LongestMatchColumnWidthStyleStrategy;
pub use crate::style::column::simple_column_width_style_strategy::SimpleColumnWidthStyleStrategy;
pub use crate::style::default_style::DefaultStyle;
...
pub struct ExcelWriter {
    ...
    template_package: Option<crate::template_write::TemplatePackage>,
    ...
    /// records are copied verbatim ([`crate::biff8::Biff8TemplatePackage`]).
    xls_template: Option<crate::biff8::Biff8TemplatePackage>,
    ...
    gzip_spills: HashMap<String, crate::gzip_spill::GzipSheetDataWriter>,
    /// Last finished gzip spill snapshot (for tests / observability).
    last_gzip_spill: Option<crate::gzip_spill::GzipSpillSnapshot>,
}
...
impl ExcelWriter {
    ...
    pub fn has_template_configured(&self) -> bool {
        crate::template_write::has_template(
    ...
    ///
    /// Populated when [`Self::finish`] closes active [`crate::gzip_spill::GzipSheetDataWriter`]s.
    #[must_use]
    pub const fn last_gzip_spill_snapshot(&self) -> Option<&crate::gzip_spill::GzipSpillSnapshot> {
    ...
    fn start(&mut self) -> Result<()> {
        ...
        if crate::template_write::has_template(
            ...
            if self.is_xls() {
                ...
                let bytes = crate::template_write::load_template_bytes(
                    self.template_file.as_deref(),
                    self.template_bytes.as_deref(),
                )?;
                if !crate::biff8::looks_like_xls(&bytes) {
                    return Err(ExcelError::Format(
                        "xls with_template requires an OLE .xls workbook".to_owned(),
                    ));
                }
                let package = crate::biff8::Biff8TemplatePackage::from_bytes(&bytes)?;
                ...
                crate::template_write::validate_template_source(
                    self.template_file.as_deref(),
                    self.template_bytes.as_deref(),
                )?;
                let bytes = crate::template_write::load_template_bytes(
                ...
                if self.use_legacy_template_seed {
                    ...
                    let package = crate::template_write::TemplatePackage::from_bytes(&bytes)?;
                    for (index, name) in package.sheet_names()?.into_iter().enumerate() {
                        let next_row = package.next_row_for_sheet(&name)?;
                        self.sheet_indexes.insert(index, name.clone());
                        self.template_pending_rows.insert(name, next_row);
    ...
    I: IntoIterator<Item = T>,
{
    ...
    let mut package = crate::biff8::Biff8TemplatePackage::from_bytes(&bytes)?;
    let sheet_names = package.sheet_names();
    let (target_index, target_name, create_new) = crate::template_write::resolve_package_target(
        &sheet_names,
        options.sheet_index,
        &options.sheet_name,
    );
...
pub(crate) fn write_sheet_to_workbook_with_gzip<T, I>(
    ...
    rows: I,
    handlers: &mut [Box<dyn WriteHandler>],
    gzip_spill: Option<&mut crate::gzip_spill::GzipSheetDataWriter>,
    skip_sheet_create_callbacks: bool,
    holder_scope: Option<&HandlerHolderScope>,
) -> Result<WriteProgress>
where
    ...
    I: IntoIterator<Item = T>,
{
    ...
    let mut package = crate::template_write::TemplatePackage::from_bytes(&bytes)?;
    let sheet_names = package.sheet_names()?;
    let (target_index, target_name, create_new) = crate::template_write::resolve_package_target(
        &sheet_names,
        options.sheet_index,
...
pub(crate) fn save_template_package(
    package: &crate::template_write::TemplatePackage,
    path: &Path,
    output: Option<&mut (dyn Write + Send)>,
    password: Option<&str>,
) -> Result<()> {
...
pub(crate) fn append_rows_to_worksheet_with_gzip_and_context<T, I>(
    ...
    write_head: bool,
    metadata: &ExcelWriteMetadata,
    mut gzip_spill: Option<&mut crate::gzip_spill::GzipSheetDataWriter>,
    holder_scope: Option<&HandlerHolderScope>,
) -> Result<WriteProgress>
where
    T: ExcelRow,
