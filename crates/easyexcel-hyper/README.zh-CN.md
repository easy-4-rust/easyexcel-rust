...
共享 EasyExcel Web 运行时的 Hyper 原生集成。

> 版本线：0.1.1 · Rust 1.88+ · Edition 2024 · Apache-2.0

## 职责

- 以 `ExcelRequest<T>` 与 `ExcelResponse<T>` 提供 Hyper 原生请求与流式响应桥接。
- 把共享策略与问题详情映射到 Hyper 传输原语。
...
主要公共 API：`ExcelRequest, ExcelResponse, ExcelHyperError, ExcelWebPolicy, ExcelWebRuntime`。
...
业务规则、解析与资源限制位于 `easyexcel-web`；本 crate 只负责 Hyper 传输适配。可运行示例位于仓库的 `examples/hyper`。
