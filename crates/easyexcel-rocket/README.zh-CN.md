...
面向 Rocket 的 EasyExcel 原生请求提取与响应适配器。
...
`easyexcel-rocket` 只负责把 Rocket 传输类型桥接到 `easyexcel-web`。上传落盘、资源限制、行流背压、取消、超时、临时文件清理与稳定错误协议均由共享内核实现，避免各框架产生不同语义。

原生集成方式：`FromData` data guard and Rocket `Responder`；运行时传递方式：managed `State<ExcelWebRuntime>`。

## 一览

```text
HTTP 请求 -> easyexcel-rocket -> easyexcel-web -> 类型化行 / 流式响应
```
...
```mermaid
flowchart LR
    Request["Rocket 请求"] --> Adapter["easyexcel-rocket"]
    Adapter --> Import["easyexcel-web / ExcelImport"]
    Import --> Rows["ExcelRows<T> / 背压"]
    Rows --> Handler["业务 Handler"]
    Handler --> Export["ExcelExport<T>"]
    Export --> Response["Rocket 响应"]
```

适配器不得重新实现 Excel 解析、写入或资源策略。业务行在有界通道中消费，下载通过异步文件流交给 Rocket。

## 能力与边界

| easyexcel-rocket 做什么 | easyexcel-rocket 不做什么 |
|:---|:---|
| `FromData` data guard 提取类型化背压行流 | 上传落盘 / 资源限制 / 超时（在 `easyexcel-web` 中） |
| Rocket `Responder` 流式下载 XLSX/XLS/CSV | 业务校验、鉴权或持久化 |
| `ExcelRocketError` 映射到 Rocket 错误协议 | 重新实现 Excel 解析或写入 |
| Managed `State<ExcelWebRuntime>` 注入 | TUI / HTML 表单处理 |

## 能力矩阵

| 能力 | 状态 | 实现 |
|:---|:---|:---|
| `ExcelRequest<T>` | 可用 | 原生 Rocket 请求提取，返回类型化背压行流。 |
| `ExcelResponse<T>` | 可用 | 发送响应头前完成受控文件生成，再异步传输。 |
| 资源与并发 | 共享 | `ExcelWebPolicy` + `ExcelWebRuntime` |
| 错误协议 | 稳定 | `ExcelRocketError` + `ExcelProblemDetails` |
...
[dependencies]
easyexcel = "0.1.3"
easyexcel-rocket = "0.1.3"
```

所有工作簿 API 仍通过 `easyexcel::...` 使用；只有 Rocket 原生 data guard、responder 与错误类型来自本适配器。适配器依赖 `easyexcel`，门面反向重导出会形成循环依赖。两个 crate 必须保持同一发布线。

## 来自 examples 的用法

可运行示例位于 [`examples/rocket`](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/rocket)。默认端口：**8000**（Rocket 默认）。

```bash
cargo run -p example-rocket
...
`ExcelResponse::prepare` 在返回 Rocket 响应前完成生成和限制校验；成功后响应体从临时文件异步读取，不把完整文件复制到内存。
...
- `ExcelRocketError` 把共享错误映射为框架原生 rejection/error/response。
...
- 完整可运行服务位于 `examples/rocket`，共享一致性断言位于 `tests/easyexcel-web-conformance`。
...
禁止形成 `easyexcel-web -> easyexcel-rocket` 或 `easyexcel -> easyexcel-rocket` 的反向依赖。
...
| 可运行集成 | [`examples/rocket`](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/rocket) |
...
- [可运行示例](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/rocket)
