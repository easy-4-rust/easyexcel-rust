...
| What easyexcel-rocket does | What easyexcel-rocket does NOT do |
|:---|:---|
| `FromData` data guard for typed backpressured row stream | Upload spooling / resource limits / timeouts (in `easyexcel-web`) |
| Rocket `Responder` for streaming XLSX/XLS/CSV download | Business validation, authorization or persistence |
| `ExcelRocketError` mapping to Rocket error protocol | Reimplementing spreadsheet parsing or writing |
| Managed `State<ExcelWebRuntime>` injection | TUI / HTML form handling |
...
All workbook APIs remain under `easyexcel::...`; only Rocket-native data guard, responder and error types come from this adapter. The adapter depends on `easyexcel`, so facade-side re-export would create a cycle. Keep both crates on the same release line.

## Usage from examples

The runnable example is in [`examples/rocket`](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/rocket). Default port: **8000** (Rocket default).
