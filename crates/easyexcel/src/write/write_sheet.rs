...
use crate::core::{Converter, NullableObjectConverter};
...
impl<T> WriteSheet<T> {
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 从完整选项集创建工作表元数据。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 使用提供的名称创建工作表元数据。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 使用 Java 风格的零基工作表编号创建工作表元数据。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 注册覆盖工作簿注册的工作表级转换器。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 注册可以接收缺失的 `Option<T>` 值的工作表级转换器。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 添加绝对合并单元格范围。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 为零基物理列设置显式宽度。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 替换表头样式。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 为所有内容行使用一种样式。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 循环使用提供的样式。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 注册重复数据行合并策略。
    #[must_use]
    ...
    /// 对应 Java：com.alibaba.excel.write.metadata.WriteSheet。 使用动态多级表头路径替换派生的表头。
    #[must_use]
...
mod tests {
    ...
    use crate::core::{Converter, DynamicRow};
