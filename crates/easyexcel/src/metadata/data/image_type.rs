...
impl ImageType {
    /// Java `values()` 的声明顺序。
    pub const ALL: [Self; 6] = [Self::Emf, Self::Wmf, Self::Pict, Self::Jpeg, Self::Png, Self::Dib];
    /// Java 枚举常量名。
    #[must_use] pub const fn java_name(self) -> &'static str {
        match self {
            Self::Emf => "PICTURE_TYPE_EMF", Self::Wmf => "PICTURE_TYPE_WMF",
            Self::Pict => "PICTURE_TYPE_PICT", Self::Jpeg => "PICTURE_TYPE_JPEG",
            Self::Png => "PICTURE_TYPE_PNG", Self::Dib => "PICTURE_TYPE_DIB",
        }
    }
    /// Java `getValue()` 使用的 POI 图片类型编号。
    #[must_use] pub const fn get_value(self) -> i32 {
        match self { Self::Emf => 2, Self::Wmf => 3, Self::Pict => 4, Self::Jpeg => 5, Self::Png => 6, Self::Dib => 7 }
    }
}
