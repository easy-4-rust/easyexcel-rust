...
/// 对应 Java：com.alibaba.excel.metadata.data.ReadCellData。 A no-model row keyed by zero-based physical column index.
///
...
impl DynamicRow {
    ...
    /// 对应 Java：com.alibaba.excel.metadata.data.ReadCellData。 Returns a value by zero-based physical column index. (Java `Map.get(Integer)`)
    #[must_use]
    pub fn get(&self, column_index: usize) -> Option<&DynamicValue> {
        self.0.get(&column_index)
    }

    /// 对应 Java：com.alibaba.excel.metadata.data.ReadCellData。 Consumes the row and returns its ordered values.
    #[must_use]
