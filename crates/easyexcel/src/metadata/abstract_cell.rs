...
impl AbstractCell {
    ...
    #[must_use]
    pub const fn get_row_index(&self) -> Option<i32> { self.row_index }

    /// 设置行下标。对应 Java：`AbstractCell#setRowIndex`。
    pub const fn set_row_index(&mut self, row_index: Option<i32>) { self.row_index = row_index; }

    /// 返回列下标。对应 Java：`AbstractCell#getColumnIndex`。
    #[must_use]
    pub const fn get_column_index(&self) -> Option<i32> { self.column_index }

    /// 设置列下标。对应 Java：`AbstractCell#setColumnIndex`。
    pub const fn set_column_index(&mut self, column_index: Option<i32>) { self.column_index = column_index; }
}
