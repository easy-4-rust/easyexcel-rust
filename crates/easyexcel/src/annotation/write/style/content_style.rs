...
impl Default for ContentStyle {
    fn default() -> Self {
        Self {
            ...
            quote_prefix: BooleanEnum::Default,
            horizontal_alignment: HorizontalAlignmentEnum::Default,
            wrapped: BooleanEnum::Default,
            vertical_alignment: VerticalAlignmentEnum::Default,
            rotation: -1,
            indent: -1,
            border_left: BorderStyleEnum::Default,
            border_right: BorderStyleEnum::Default,
            border_top: BorderStyleEnum::Default,
            border_bottom: BorderStyleEnum::Default,
            left_border_color: -1,
            right_border_color: -1,
            top_border_color: -1,
            bottom_border_color: -1,
            fill_pattern_type: FillPatternTypeEnum::Default,
            fill_background_color: -1,
            fill_foreground_color: -1,
            shrink_to_fit: BooleanEnum::Default,
        }
    }
}
impl ContentStyle {
    ...
    pub fn to_write_cell_style(self) -> WriteCellStyle {
        ...
        WriteCellStyle {
            ...
            quote_prefix: self.quote_prefix.value(),
            horizontal_alignment: self.horizontal_alignment.poi_horizontal_alignment(),
            wrapped: self.wrapped.value(),
            vertical_alignment: self.vertical_alignment.poi_vertical_alignment_enum(),
            rotation: (self.rotation >= 0).then_some(self.rotation),
            indent: u8::try_from(self.indent).ok(),
            border_left: self.border_left.poi_border_style(),
            border_right: self.border_right.poi_border_style(),
            border_top: self.border_top.poi_border_style(),
            border_bottom: self.border_bottom.poi_border_style(),
            left_border_color: color(self.left_border_color),
            right_border_color: color(self.right_border_color),
            top_border_color: color(self.top_border_color),
            bottom_border_color: color(self.bottom_border_color),
            fill_pattern: self.fill_pattern_type.poi_fill_pattern_type(),
            fill_background_color: color(self.fill_background_color),
            fill_foreground_color: color(self.fill_foreground_color),
            shrink_to_fit: self.shrink_to_fit.value(),
            data_format: u8::try_from(self.data_format)
                .ok()
                .map(ExcelDataFormat::Builtin),
            font: None,
        }
    }
...
mod tests {
    ...
    fn to_write_cell_style_with_values() {
        ...
        assert_eq!(
            wcs.horizontal_alignment,
            Some(crate::ExcelHorizontalAlignment::Left)
        );
        assert_eq!(
            wcs.vertical_alignment,
            Some(crate::ExcelVerticalAlignment::Bottom)
        );
