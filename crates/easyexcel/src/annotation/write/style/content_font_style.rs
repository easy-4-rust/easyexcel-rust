...
use crate::{
    BooleanEnum, ExcelColor, ExcelFontScript, ExcelFontStyle, ExcelUnderline, FontProperty,
};
...
impl Default for ContentFontStyle {
    fn default() -> Self {
        Self {
            ...
            italic: BooleanEnum::Default,
            strikeout: BooleanEnum::Default,
            color: -1,
            type_offset: -1,
            underline: -1,
            charset: -1,
            bold: BooleanEnum::Default,
        }
    }
}
impl ContentFontStyle {
    ...
    pub fn to_property(&self) -> FontProperty {
        FontProperty::build(ExcelFontStyle {
            ...
            font_height_in_points: (self.font_height_in_points >= 0)
                .then_some(f64::from(self.font_height_in_points)),
            italic: self.italic.value(),
            strikeout: self.strikeout.value(),
            color: u32::try_from(self.color).ok().map(ExcelColor::java_or_rgb),
            type_offset: match self.type_offset {
                0 => Some(ExcelFontScript::None),
                1 => Some(ExcelFontScript::Superscript),
                2 => Some(ExcelFontScript::Subscript),
                _ => None,
            },
            underline: match self.underline {
                0 => Some(ExcelUnderline::None),
                1 => Some(ExcelUnderline::Single),
                2 => Some(ExcelUnderline::Double),
                33 => Some(ExcelUnderline::SingleAccounting),
                34 => Some(ExcelUnderline::DoubleAccounting),
                _ => None,
            },
            charset: u8::try_from(self.charset).ok(),
            bold: self.bold.value(),
        })
    }
}
