...
pub struct ColumnWidth { value: i32 }

impl Default for ColumnWidth { fn default() -> Self { Self { value: -1 } } }

impl ColumnWidth {
    /// 创建 Java 默认参数对象。
    #[must_use] pub const fn new() -> Self { Self { value: -1 } }
    /// 返回列宽。
    #[must_use] pub const fn value(&self) -> i32 { self.value }
    /// 设置列宽。
    pub const fn set_value(&mut self, value: i32) { self.value = value; }
