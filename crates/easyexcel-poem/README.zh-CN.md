...
| easyexcel-poem 做什么 | easyexcel-poem 不做什么 |
|:---|:---|
| `FromRequest` extractor 提取类型化背压行流 | 上传落盘 / 资源限制 / 超时（在 `easyexcel-web` 中） |
| `IntoResponse` responder 流式下载 XLSX/XLS/CSV | 业务校验、鉴权或持久化 |
| `ExcelPoemError` 映射到 Poem 错误协议 | 重新实现 Excel 解析或写入 |
| `Data<&ExcelWebRuntime>` + `AddData` 注入 | TUI / HTML 表单处理 |
...
可运行示例位于 [`examples/poem`](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/poem)。默认端口：**8083**。

```bash
cargo run -p example-poem
# 监听 http://127.0.0.1:8083
