...
面向 Axum 的 EasyExcel 原生请求提取与响应适配器。
...
`easyexcel-axum` 只负责把 Axum 传输类型桥接到 `easyexcel-web`。上传落盘、资源限制、行流背压、取消、超时、临时文件清理与稳定错误协议均由共享内核实现，避免各框架产生不同语义。

原生集成方式：`FromRequest` extractor and `IntoResponse` responder；运行时传递方式：`State<ExcelWebRuntime>` / `FromRef`。

## 一览

```text
HTTP 请求 -> easyexcel-axum -> easyexcel-web -> 类型化行 / 流式响应
```
...
```mermaid
flowchart LR
    Request["Axum 请求"] --> Adapter["easyexcel-axum"]
    Adapter --> Import["easyexcel-web / ExcelImport"]
    Import --> Rows["ExcelRows<T> / 背压"]
    Rows --> Handler["业务 Handler"]
    Handler --> Export["ExcelExport<T>"]
    Export --> Response["Axum 响应"]
```

适配器不得重新实现 Excel 解析、写入或资源策略。业务行在有界通道中消费，下载通过异步文件流交给 Axum。

## 能力与边界

| easyexcel-axum 做什么 | easyexcel-axum 不做什么 |
|:---|:---|
| `FromRequest` extractor 提取类型化背压行流 | 上传落盘 / 资源限制 / 超时（在 `easyexcel-web` 中） |
| `IntoResponse` responder 流式下载 XLSX/XLS/CSV | 业务校验、鉴权或持久化 |
| `ExcelRejection` 映射到 Axum 错误协议 | 重新实现 Excel 解析或写入 |
| `State<ExcelWebRuntime>` / `FromRef` 注入 | TUI / HTML 表单处理 |

## 能力矩阵

| 能力 | 状态 | 实现 |
|:---|:---|:---|
| `ExcelRequest<T>` | 可用 | 原生 Axum 请求提取，返回类型化背压行流。 |
| `ExcelResponse<T>` | 可用 | 发送响应头前完成受控文件生成，再异步传输。 |
| 资源与并发 | 共享 | `ExcelWebPolicy` + `ExcelWebRuntime` |
| 错误协议 | 稳定 | `ExcelRejection` + `ExcelProblemDetails` |
...
所有工作簿 API 仍通过 `easyexcel::...` 使用；只有 Axum 原生 extractor、responder 与 rejection 类型来自本适配器。适配器依赖 `easyexcel`，门面反向重导出会形成循环依赖。两个 crate 必须保持同一发布线。

## 来自 examples 的用法

可运行示例位于 [`examples/axum`](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/axum)。默认端口：**8080**（可通过 `PORT` 环境变量配置）。
...
`ExcelResponse::prepare` 在返回 Axum 响应前完成生成和限制校验；成功后响应体从临时文件异步读取，不把完整文件复制到内存。
...
- `ExcelRejection` 把共享错误映射为框架原生 rejection/error/response。
...
- 完整可运行服务位于 `examples/axum`，共享一致性断言位于 `tests/easyexcel-web-conformance`。
...
禁止形成 `easyexcel-web -> easyexcel-axum` 或 `easyexcel -> easyexcel-axum` 的反向依赖。
...
| 错误映射 | [`src/excel_rejection.rs`](src/excel_rejection.rs) |
| 可运行集成 | [`examples/axum`](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/axum) |
...
- [API 文档](https://docs.rs/easyexcel-axum)
...
- [可运行示例](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/axum)
