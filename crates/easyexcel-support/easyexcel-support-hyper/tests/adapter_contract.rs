...
//! 的下载 / 上传 / 失败降级语义（Hyper 版）。
//!
//! 与 easyexcel-support-axum 的 tests.rs 断言 1:1 对齐，仅将 axum 类型替换为
//! hyper 1.x 类型（`hyper::Response<ResponseBody>`）。Hyper 无 multipart
//! 提取器，上传 API 只接收字节，因此契约测试直接构造字节而非 HTTP multipart 请求。
...
use easyexcel_support_hyper::{
    ResponseBody, excel_download_error_response, excel_download_or_json_response,
    excel_download_response, excel_download_response_from_bytes, excel_xlsx_attachment_headers,
    extension_from_path, read_upload_sync, read_upload_with_listener, write_rows_to_bytes,
    write_upload_temp,
};
...
/// 对应 Java：端到端确认 `excel_download_or_json_response` 的字节响应成功路径。
///
...
/// 将 Hyper 响应体读为字节（测试辅助）。
///
/// `http_body_util::BodyExt::collect` 以全限定形式调用，直接消费
/// [`ResponseBody`]（`Full<Bytes>`）返回的 `Collected`。
