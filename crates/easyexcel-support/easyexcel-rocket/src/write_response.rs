...
pub fn excel_download_response_from_bytes(
    ...
    builder
        .status(Status::Ok)
        .sized_body(length, Cursor::new(bytes));
    for header in excel_xlsx_attachment_headers(file_name) {
        builder.header(header);
    }
    Ok(builder.finalize())
}

/// 一步完成写入并返回 Rocket XLSX 附件响应。
///
...
/// 下载失败时返回 JSON 体（Rocket [`Response`]）。
///
/// 对应 Java `WebTest.downloadFailedUsingJson` 的 `catch` 分支与 Fastjson 输出：
/// 500 + `application/json; charset=utf-8` + `{"status":"failure",...}`。
#[must_use]
pub fn excel_download_error_response(body: ExcelDownloadErrorBody) -> Response<'static> {
    ...
    builder
        ...
        .sized_body(length, Cursor::new(json));
    builder.finalize()
}
