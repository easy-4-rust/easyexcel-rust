//! Salvo Web 适配器契约测试：对应 Java `com.alibaba.easyexcel.test.demo.web.WebTest`
//! 的下载 / 上传 / 失败降级语义（与 easyexcel-support-axum 的 tests.rs 1:1 对齐）。
...
/// 读取 Salvo 响应体为字节（测试辅助）。
///
/// 附件 / JSON 体均由 `Response::body` 写入，落为 `ResBody::Once`；
/// `Chunks` 分支仅防御性保留。
fn body_bytes(mut response: salvo::Response) -> Vec<u8> {
    match response.take_body() {
        salvo::http::ResBody::Once(bytes) => bytes.to_vec(),
        salvo::http::ResBody::Chunks(chunks) => chunks
            .into_iter()
            .flat_map(|bytes| bytes.to_vec())
            .collect(),
        other => panic!("unexpected salvo body kind: {other:?}"),
    }
}
...
fn excel_download_response_from_bytes_sets_headers_and_status() {
    ...
    assert_eq!(body_bytes(response).len(), bytes.len());
}
...
fn excel_download_error_response_returns_json_500() {
    ...
    let value: Value = serde_json::from_slice(&body_bytes(response)).expect("json");
    assert_eq!(value["status"], json!("failure"));
    assert_eq!(value["message"], json!("下载文件失败stream closed"));
}
