...
| easyexcel-warp 做什么 | easyexcel-warp 不做什么 |
|:---|:---|
| 类型化 `Filter` 提取背压行流 | 上传落盘 / 资源限制 / 超时（在 `easyexcel-web` 中） |
| `Reply` 流式下载 XLSX/XLS/CSV | 业务校验、鉴权或持久化 |
| `ExcelWarpRejection` + `recover_excel_rejection` 错误映射 | 重新实现 Excel 解析或写入 |
| `excel_request` filter 捕获运行时 | TUI / HTML 表单处理 |
...
可运行示例位于 [`examples/warp`](https://github.com/easy-4-rust/easyexcel-rust/tree/main/examples/warp)。默认端口：**8085**。

```bash
cargo run -p example-warp
# 监听 http://127.0.0.1:8085
