...
//! ```
//!
//! Warp 0.4 的 `Reply` 响应类型为 `warp::reply::Response`（
//! `http::Response<warp 内部 Body>`，warp 内部 Body 包装
...
/// 对应 Java：无直接对应对象；Rust 架构扩展。 由已生成的 XLSX 字节构建 Warp 附件响应。
///
/// # Errors
///
...
pub fn excel_download_error_response(body: ExcelDownloadErrorBody) -> warp::reply::Response {
    let json = serde_json::to_string(&body).unwrap_or_else(|_| {
        r#"{"status":"failure","message":"下载文件失败JSON序列化错误"}"#.to_owned()
    });
    let mut response = warp::reply::Response::new(json.into());
